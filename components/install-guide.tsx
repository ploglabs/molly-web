...
export function InstallGuide() {
  ...
  const updateCommands: Record<Platform, string> = {
    macos: "brew upgrade ploglabs/tap/molly",
    linux: "yay -Syu molly-bin",
    windows: "scoop update molly",
    go: "go install github.com/ploglabs/molly-terminal/cmd/molly@latest",
  };
  ...
  const displayUpdateCommands: Record<Platform, React.ReactNode> = {
    macos: (
      <code className="text-zinc-300 block font-mono text-xs">
        brew upgrade ploglabs/tap/molly
      </code>
    ),
    linux: (
      <div className="space-y-2">
        <div>
          <span className="text-[9px] text-zinc-600 block uppercase font-mono">// ARCH LINUX</span>
          <code className="text-zinc-300 block font-mono text-xs">yay -Syu molly-bin</code>
        </div>
        <div>
          <span className="text-[9px] text-zinc-600 block uppercase font-mono mt-1">// DEBIAN / UBUNTU</span>
          <code className="text-zinc-300 block font-mono text-xs">
  ...
  const copyToClipboard = () => {
    const text = action === "install" ? commands[platform] : updateCommands[platform];
    ...
    <div className="w-full max-w-md font-mono mt-2">
      ...
      <div className="flex flex-col gap-2 mb-3">
        ...
        <div className="flex border border-zinc-800 bg-zinc-950 p-1">
          {(["install", "update"] as ActionType[]).map((a) => (
            <button
              key={a}
              onClick={() => {
                setAction(a);
      ...
      <div className="relative border border-zinc-800 bg-zinc-950 p-4 min-h-[72px] flex items-center justify-between group">
        <div className="pr-16 w-full overflow-x-auto no-scrollbar">
          <span className="text-[10px] text-zinc-500 block uppercase font-mono mb-2">// {action.toUpperCase()}</span>
          {action === "install" ? displayCommands[platform] : displayUpdateCommands[platform]}
        </div>
        <button
